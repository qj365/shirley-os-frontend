    ...
    const fetchProductData = async () => {
      try {
        ...
        if (categoryProducts.length > 0) {
          ...
          const mainProduct = categoryProducts.find((product: MedusaProduct) => product.id === productId);
          
          if (mainProduct) {
            setProduct(mainProduct);
            // Filter out the main product from related products
            setAdditionalProducts(categoryProducts.filter((product: MedusaProduct) => product.id !== productId));
          } else {
