...
export const debugLog = (operation: string, data: any): void => {
...
export const createSubscriptionItemsFromCart = (cart: any): SubscriptionItem[] => {
...
export const createGuestCustomerFromCart = (cart: any): GuestCustomer | null => {
  if (!cart?.email) {
    return null
  }
...
export const getProductIdFromCart = (cart: any): string | null => {
  if (!cart?.items || cart.items.length === 0) {
    return null
  }
