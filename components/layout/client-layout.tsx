...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  const showNavbar = () => {
    const showPaths = ['/', '/checkout', '/shop', '/wholesale', '/about', '/signin', '/recipe', '/recipes', '/dashboard', '/order-confirmation', '/faq'];
      ...
      <body className="relative">
        ...
        <CartProvider>
          <div className={`transition-all duration-300`}>
            ...
            {children}
            {!isAuthPage() && !pathname?.includes('/shop/product_detail') && !pathname?.includes('/checkout') && <Footer />}
          </div>
        </CartProvider>
      </body>
    </html>
