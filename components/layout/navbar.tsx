...
const getRouteStyles = (pathname: string) => {
  ...
  const isWhiteMode =
    ['/shop', '/wholesale', '/about', '/faq', '/cooking-classes'].includes(pathname) ||
    isCookingClassBookingPage;

  return {
    textColor: isWhiteMode ? 'text-white' : 'text-black',
    logoSrc: isWhiteMode ? '/image/Logo_white.png' : '/image/Logo_Charcoal_Black.png',
...
const NavigationLinks = ({
  ...
  const linkClass = (path: string) =>
    `font-semibold text-lg lg:text-xl xl:text-2xl transition-colors hover:text-red-600 ${
      pathname === path || pathname.startsWith(`${path}/`) ? 'text-red-600' : textColor
    }`;
  ...
  return (
    <>
      ...
      <Link href="/wholesale" className={linkClass('/wholesale')} onClick={onLinkClick}>
        Wholesale
      </Link>
      <Link href="/cooking-classes" className={linkClass('/cooking-classes')} onClick={onLinkClick}>
        Classes
      </Link>
...
const AuthSection = ({
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <Logout
        textColor={
          isWhiteMode ? 'text-white hover:text-gray-300' : 'text-gray-700 hover:text-gray-900'
        }
      />
    </div>
  );
...
const initCustomNavStyles = {
  ...
  buttonStyles: 'bg-black text-white hover:bg-black hover:text-white border-black',
  isWhiteMode: false,
};
      ...
      >
        <div className="mx-auto flex w-full items-center justify-between transition-all duration-500">
          ...
          <nav className="hidden items-center gap-4 lg:flex lg:gap-6 xl:gap-8">
            <NavigationLinks pathname={pathname} textColor={customNavStyles.textColor} />
            <CartButton
              totalQuantity={totalQuantity}
              size={50}
              className="rounded-full hover:rounded-r-full hover:bg-red-50 hover:opacity-80"
            />
          </nav>
          ...
          <div className="flex items-center gap-4 lg:hidden">
            ...
            <button
              onClick={toggleMenu}
              className={`min-w-[40px] text-2xl focus:outline-none ${customNavStyles.isWhiteMode ? 'text-white' : 'text-black'}`}
        ...
        {/* Mobile Menu */}
        <div
          className={`absolute left-0 z-50 mt-2 w-full overflow-x-hidden border border-gray-200 bg-[#fffbf0] transition-all duration-300 ease-in-out lg:hidden ${isMenuOpen ? 'max-h-[600px] px-6 pb-4 opacity-100' : 'max-h-0 px-6 py-0 opacity-0'}`}
        >
          <nav
