...
export default function MainSection({ product, additionalProducts}: MainSectionProps) {
  ...
  const isOutOfStock = firstVariant?.inventory_quantity === 0;
  ...
  const nutritionalInfo = metadata.nutritional_info as string | Record<string, any>;
  const storageInstructions = metadata.storage_instructions as string;
  const shelfLife = metadata.shelf_life as string;
  const additionalInfo = metadata.additional_info as string | Record<string, any>;
